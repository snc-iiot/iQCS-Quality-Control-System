...
export class ProductivityService {
  ...
  async create(
    ...
    try {
      ...
      const record = {
        ...
        ng_quantity: input.ng_quantity ?? 0,
    ...
    try {
      ...
      const results =
        await this.ProductivityLoggingRepository.createQueryBuilder('t1')
          ...
          .leftJoin('tb_users', 't3', 't1.creator_id = t3.user_id')
          .select(
            `t1.datetime,t1.process,t1.part_code,t2.part_name,t3.name as creator_name
          ,concat(to_char(t1.datetime, 'HH24:MI'), ' - ', to_char(t1.datetime + interval '1 hour', 'HH24:MI'))  as time_slot
          ,(case when extract(hour from datetime) >= 8 and extract(hour from datetime) < 20 then 'DAY' else 'NIGHT' end) as shift
          ,sum(t1.quantity) over (partition by t1.datetime,t1.process,t1.part_code) as quantity
          ,sum(coalesce(t1.ng_quantity, 0)) over (partition by t1.datetime,t1.process,t1.part_code) as ng_quantity
  ...
  async delete(input: DeleteProductivityDto): Promise<TServiceResponse> {
    try {
