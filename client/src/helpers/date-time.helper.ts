...
/**
 * The function `renderFormattedDateWithTime` takes a date input, parses and validates it, then formats
 * it in the "MMM dd, yyyy HH:mm" format before returning the formatted date as a string or null if the
 * input is invalid.
 * @param {string | Date | undefined | null} date - The `renderFormattedDateWithTime` function takes a
 * parameter `date`, which can be a string, a Date object, undefined, or null. The function first
 * parses the date to check if it is valid, then formats the date in the "MMM dd, yyyy HH:mm" format.
 * If
 * @returns The function `renderFormattedDateWithTime` returns a formatted date string in the format
 * "MMM dd, yyyy HH:mm" if the input date is valid. If the input date is undefined, null, or invalid,
 * the function returns null.
 */
export const renderFormattedDateWithTime = (date: string | Date | undefined | null): string | null => {
  // Parse the date to check if it is valid
  const parsedDate = getDate(date);
  // return if undefined
  if (!parsedDate) return null;
